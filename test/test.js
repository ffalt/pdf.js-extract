...
const pdfDirectory = _resolve(__dirname, "../example/");

const sampleFile = join(pdfDirectory, "example.pdf");
const sampleOutput = JSON.parse(
  readFileSync(join(pdfDirectory, "example-output.json")).toString(),
);
const sampleEncryptedFile = join(pdfDirectory, "encrypted.pdf");
const sampleEncryptedOutput = JSON.parse(
  readFileSync(join(pdfDirectory, "encrypted-output.json")).toString(),
);
const sampleCmapFile = join(pdfDirectory, "example-cmap.pdf");
const sampleCmapOutput = JSON.parse(
  readFileSync(join(pdfDirectory, "example-cmap-output.json")).toString(),
);
...
describe("PDFExtract", () => {
  describe("#extractBuffer()", () => {
    it("should extract pdf buffer without error", (done) => {
      ...
      const buffer = readFileSync(sampleFile);
    ...
    it("should extract encrypted pdf buffer without error", (done) => {
      const extract = new PDFExtract();
      const buffer = readFileSync(sampleEncryptedFile);
